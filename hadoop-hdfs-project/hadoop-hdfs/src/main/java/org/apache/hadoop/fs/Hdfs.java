...
public class Hdfs extends AbstractFileSystem {
  ...
  abstract private class DirListingIterator<T extends FileStatus>
    ...
    private DirListingIterator(Path p, boolean needLocation)
      ...
      thisListing = dfs.listPaths(src, HdfsFileStatus.EMPTY_NAME, needLocation);
    ...
    public boolean hasNext() throws IOException {
      ...
      if (i >= thisListing.getPartialListing().length &&
        ...
        thisListing =
            dfs.listPaths(src, thisListing.getLastName(), needLocation);
        if (thisListing == null) {
     ...
     * @throws NoSuchElmentException
     *     if no more entry is available
     */
